...
public enum HKRPGDXMT {
    ...
    private static var versionFileURL: URL {
        HKRPGResources.dxmtDir.appending(path: "installed-version.txt")
    }
    ...
    private static func isInstalledFilesPresent() -> Bool {
        dxmtFiles.allSatisfy { name in
            let url = HKRPGResources.dxmtDir.appending(path: name)
    ...
    private static func clearDXMTDir() {
        let dir = HKRPGResources.dxmtDir
    ...
    public static func ensureInstalled(progress: (@Sendable (Double) -> Void)? = nil) async throws {
        ...
        let archiveLocalURL = HKRPGResources.dxmtDir.appending(path: archiveName)
    ...
    public static func applyToPrefix(prefixURL: URL) throws {
        ...
        for name in names + ["winemetal.dll"] {
            let src = HKRPGResources.dxmtDir.appending(path: name)
    ...
    public static func applyToRuntime(runtimeRoot: URL) {
        ...
        let winemetalDLLSrc = HKRPGResources.dxmtDir.appending(path: "winemetal.dll")
        let winemetalSOSrc = HKRPGResources.dxmtDir.appending(path: "winemetal.so")
    ...
    public static func revertRuntime(runtimeRoot: URL) {
        let targets = [
            ...
            runtimeRoot.appendingPathComponent("lib/wine/x86_64-windows/nvngx.dll", isDirectory: false),
